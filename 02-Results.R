...
## -- Load packages --------
Fn$quietly_load(c(
  "tidytable", "data.table", "mice", "purrr", "stringr"
))
...
## -- Imputation function ----------------
evalq({
  source(here::here("01-Scripts", "02-Multiple-Imputation.R"), local = TRUE)
}, Results)

## -- Data in the environment ----------------
evalq({
  Data <- function() {
    readRDS(here::here(INC, "Slim.rds"))
  }
  Population <- function() {
    readRDS(here::here(DATA, "00-processed", "Population.rds"))
  }
}, Results)
...
evalq({
  ...
  ## -- Get number of cases by group ----------------
  GetCases <- function(data, case_group = c("DiagYear", "Sex", "AgeGroup5", "Tstage", "AnatomicSite", "MelanomaType")) {
    Cases <- xtabs(reformulate(case_group), data = data, addNA = TRUE) %>% 
      as.data.table()
  ...
  ## -- Prepare data to calculate incidence rate ----------------
  ...
  ## -- Prepare data to calculate incidence rate by group ----------------
...
## -- Cases and Deaths in global Results environment ----------------
evalq({
  Cases <- GetCases(Data())
...
evalq({
  ...
  PersonYear[, AgeGroup := CodeMap$age_map(AgeGroup5, from = "5years")]
...
evalq({
  ...
  AgeAdjRate <- function(cases, person_year, group, std_pop = "europe", margin = NULL, scale = 1e5) {
...
evalq({
  Segmented <- function(rate_data, which = "fit", logY = TRUE) {
    ...
    group_vars <- setdiff(
      x = names(rate_data),
      y = c("N", "Year", "DiagYear", "crude.rate", "adj.rate",
...
## -- Load or run imputation ----------------
evalq({
  if (!exists("Imputed")) {
    require(mice)
    
    if (file.exists(here::here(INC, "Imputed.rds"))) {
      Imputed <- readRDS(here::here(INC, "Imputed.rds"))
    } else if (file.exists(here::here(DATA, "00-raw", "Imputed.rds"))) {
      Imputed <- readRDS(here::here(DATA, "00-raw", "Imputed.rds"))
      saveRDS(Imputed, here::here(INC, "Imputed.rds"))
    } else {
      Imputed <- Impute(Data())
      saveRDS(Imputed, here::here(INC, "Imputed.rds"))
    }
...
## -- Save Results environment ----------------
saveRDS(Results, here::here(INC, "Results.rds"))
