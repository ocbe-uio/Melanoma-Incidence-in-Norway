...
OUT <- file.path(INC_ROOT, "03-Outputs", "Tables")
...
Fn$quietly_load(c(
  ...
  "gt", "gtsummary", "kableExtra", "tidytable"
))
...
## ---- Exclusion Inclusion ----------------
ExInclusion <- function() {
  if (!exists("step_data")) {
    step_data <- readRDS(here::here(INC, "StepData.rds"))
  }
  tbl <- Fn$get_exclusion_inclusion(step_data)
  return(tbl)
}

ExInc <- ExInclusion()
ExIncTbl <- attr(ExInc, "gt_table")(ExInc)


## ---- Export: Inclusion Exclusion Tables ----------------
gt::gtsave(
  data = ExIncTbl,
  filename = here::here(OUT, "Inclusion-Exclusion.html")
)
...
evalq(
  {
    SummaryTable <- function() {
      tbl <- readRDS(here::here(TDATA, "SummaryTable.rds"))
...
## -- Export: Table-1: Charactersitic Table ----------------
cat("Saving: summary-table.html\n")
summary_table <- SelectedTables$SummaryTable()
gt::gtsave(summary_table, "summary-table.html", path = OUT)
...
evalq({
  AltTable <- function() {
    tbl <- readRDS(here::here(TDATA, "AltTable.rds"))
    tbl[["_data"]][["label"]] <- gsub(
      "Head and neck", "Head/neck", tbl[["_data"]][["label"]]
    )
    return(tbl)
...
## -- Export: Alternative Table-1 ----------------
cat("Saving: alt-table.html\n")
alt_table <- SelectedTables$AltTable()
gt::gtsave(alt_table, "alt-table.html", path = OUT)
...
evalq({
  CountComparison <- function(group = "YearCat", by = "Tstage", overall_label = "1983-2019") {
    ...
    count_comparison <- readRDS(
      here::here(TDATA, "YearCat-Tstage-CountComparison.rds")
    )
...
## -- Export: Sensitivity analysis: Count comparison ----------------
cat("Saving: count-comparison.html\n")
count_comparison <- SelectedTables$CountComparison()
gt::gtsave(count_comparison, "count-comparison.html", path = OUT)
...
evalq({
  CountComparisonByType <- function(group = "YearCat", overall_label = "1983-2019") {
    #// Following data only works for group "YearCat" and by = "Tstage"
    count_comparison <- readRDS(
      here::here(TDATA, "YearCat-MelanomaType-CountComparison.rds")
    )
    # count_comparison <- TableData$CountComparison(group = group, by = by, overall_label = overall_label)
    model_footnote <- attr(count_comparison, "footnote")
    n_imp <- attr(count_comparison, "imp")
...
## -- Export: Sensitivity analysis: Count comparison by Type ----------------
cat("Saving: count-comparison-by-type.html\n")
count_comparison_by_type <- SelectedTables$CountComparisonByType()
gt::gtsave(count_comparison_by_type, "count-comparison-by-type.html", path = OUT)
...
evalq({
  SurvComparison <- function(group = "YearCat", by = "Tstage", overall_label = "1983-2019") {
    ...
    hr_comparison <- readRDS(
      here::here(TDATA, "YearCat-Tstage-SurvComparison.rds")
    )
...
## -- Export: Sensitivity analysis: Survival comparison ----------------
cat("Saving: surv-comparison.html\n")
surv_comparison <- SelectedTables$SurvComparison()
gt::gtsave(surv_comparison, "surv-comparison.html", path = OUT)


## -- (A)APC table ----
evalq({
  AAPC <- new.env(parent = SelectedTables)
  evalq({
    TableAPC <- function(data, logY = TRUE, which = c("Pooled", "Complete", "all"),
      ...
      all_na_rows <- data %>% tidytable::select(starts_with("Trend")) %>% 
        apply(1, \(x) all(is.na(x)))
      
      data[which(all_na_rows), `Trend 1_APC` := Overall_APC]
      data[which(all_na_rows), `Trend 1_Period` := "1983–2019"]
...
evalq({
  ...
  evalq({
    BySex <- function(logY = TRUE, which = c("Pooled", "Complete", "all"), 
      ...
      which <- match.arg(which)
      type <- match.arg(type)
      out <- readRDS(here::here(TDATA, "AAPC-BySex.rds"))
  ...
  evalq({
    ByTstage <- function(logY = TRUE, which = c("Pooled", "Complete", "all"), 
      ...
      which <- match.arg(which)
      type <- match.arg(type)
      out <- readRDS(here::here(TDATA, "AAPC-ByTstage.rds"))
  ...
  evalq({
    BySexTstage <- function(logY = TRUE, which = c("Pooled", "Complete", "all"), 
      ...
      which <- match.arg(which)
      type <- match.arg(type)
      out <- readRDS(here::here(TDATA, "AAPC-BySexTstage.rds"))
  ...
  evalq({
    BySexTstageSite <- function(logY = TRUE, which = c("Pooled", "Complete", "all"), 
      ...
      out <- readRDS(here::here(TDATA, "AAPC-BySexTstageSite.rds"))
  ...
  evalq({
    BySexTstageType <- function(logY = TRUE, which = c("Pooled", "Complete", "all"), 
      ...
      out <- readRDS(here::here(TDATA, "AAPC-BySexTstageType.rds"))
  ...
  evalq({
    BySexTstageRegion <- function(logY = TRUE, which = c("Pooled", "Complete", "all"), 
      ...
      out <- readRDS(here::here(TDATA, "AAPC-BySexTstageRegion.rds"))
  ...
  evalq({
    BySexTstageSeason <- function(logY = TRUE, which = c("Pooled", "Complete", "all"), 
      ...
      which <- match.arg(which)
      type <- match.arg(type)
      out <- readRDS(here::here(TDATA, "AAPC-BySexTstageSeason.rds"))
...
evalq({
  PropTableImputed <- function() {
    dta <- readRDS(here::here(TDATA, "Counts.Rds")) %>% 
      as_tidytable()
    
    dta <- dta %>% 
      filter(Imp != "0") %>% # Take only imputed cases
      mutate(YearCat = CodeMap$year_cat(DiagYear))
    
    avg_counts <- dta %>% 
      tidytable::group_by(Imp, Sex, Tstage:MelanomaType, YearCat) %>% 
      tidytable::summarize(N = sum(N)) %>% 
      tidytable::group_by(Sex:YearCat) %>% 
      tidytable::summarize(N = mean(N))
    
    counts_with_margin <- avg_counts %>% 
      bind_rows(
        avg_counts %>% 
          groupingsets(
            .(N = sum(N)),
            by = union(attr(avg_counts, "groups"), "YearCat"),
            sets = list(c("Sex", "Tstage", "MelanomaType"))
          ) %>% 
          modify_at("AnatomicSite", forcats::fct_na_value_to_level, "Overall") %>% 
          modify_at("YearCat", forcats::fct_na_value_to_level, "1983-2019")
      ) %>% 
      ungroup() %>% 
      mutate(
        Prop = prop.table(N),
        .by = c(Sex, Tstage, AnatomicSite, YearCat)
      ) %>% 
      mutate(
      ...
      mutate(
        YearCat = gsub("-", "–", YearCat),
        Tstage = CodeMap$get_Tstage_label(Tstage)
      )
    
    out <- counts_with_margin %>% 
      pivot_wider(
        id_cols = c("Sex", "Tstage", "MelanomaType"),
        names_from = c("AnatomicSite", "YearCat"),
        values_from = "label"
      ) %>% 
      rename_with(~gsub("Head and neck", "Head/neck", .x))
    ...
    return(list(data = counts_with_margin, table = tbl))
...
gt::gtsave(prop_table_complete$table, "prop-table-complete.html", path = OUT)

cat("Saving: prop-table-imputed.html\n")
prop_table_imputed <- SelectedTables$PropTableImputed()
gt::gtsave(prop_table_imputed$table, "prop-table-imputed.html", path = OUT)

## -- Extra plots for exploring proportions ----------------
prop_imp <- SelectedTables$PropTableImputed()
prop_imp$data %>% 
  filter(MelanomaType != "Other" & AnatomicSite != "Other") %>% 
  filter(YearCat != "1983–2019") %>% 
  filter(!grepl("Lent", MelanomaType)) %>% 
  arrange(AnatomicSite, Prop, Tstage, Sex) %>% 
  ggplot(aes(YearCat, Prop, color = Tstage)) +
  ggh4x::facet_nested(
    cols = vars(MelanomaType, AnatomicSite), 
    rows = vars(Sex)
  ) +
  geom_line(aes(group = Tstage)) +
